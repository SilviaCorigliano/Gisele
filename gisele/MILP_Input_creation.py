...
from functions import distance_2d,sizing,load
...
def create_node_lines(Nodes,gisele_folder,case_study,crs):
    ...
    folder_clusters=case_folder+ 'Output/Clusters/'
    ...
    for n_cluster in range(len(clusters)):
        ...
        try:
            Lines = gpd.read_file(folder_clusters+cluster+'/grids/Grid_substations.shp')
        ...
        if n_cluster==0:
            all_substations=Nodes[Nodes_clusters.Substation==1]
        else:
            add_nodes=Nodes_clusters[Nodes_clusters.Substation == 1]
            all_substations=all_substations.append(add_nodes)
    ...
    if not os.path.exists(case_folder+'Input/MILP'):
        os.makedirs(case_folder+'Input/MILP')
    if not os.path.exists(case_folder+'Input/MILP/all_data'):
        os.makedirs(case_folder+'Input/MILP/all_data')
    New_Lines.to_csv(case_folder+'Input/MILP/all_data/Lines_clusters.csv',index=False)
    All_lines_marked.to_csv(case_folder + 'Input/MILP/all_data/Lines_marked.csv',index=False)
    All_lines_marked=gpd.GeoDataFrame(All_lines_marked,geometry=All_lines_marked.geometry,crs=crs)
    All_lines_marked.to_file(case_folder + 'Input/MILP/all_data/Lines_'
                                           'marked',index=False)
    all_substations['Population']=[ceil(i) for i in all_substations['Population'].to_list()]
    all_substations[['X','Y','ID','Weight','Cluster','MV_Power','Population','Substation','geometry']].to_csv(case_folder+'Input/MILP/all_data/All_Nodes.csv')

def create_connections(all_points,Primary_substations,gisele_folder,case_study,line_bc,resolution,crs,Roads,tolerance_outside):
    ...
    if Roads:
        ROADS = gpd.read_file(case_folder + '/Input/Roads.shp')
        ROADS = ROADS.to_crs(crs)
        ROADS = ROADS.geometry.simplify(tolerance=tolerance_outside)
    for i in range(1,n_clusters+1):
        for j in range(i + 1, n_clusters+1, 1):
            ...
            if Roads and dist< 30*resolution:
                ...
                roads=gpd.clip(ROADS,bubble)
                roads = gpd.GeoDataFrame(geometry=roads)
                roads = MultiLine_to_Line(roads)

                gdf_roads, roads_segments = create_roads(roads, all_points)
                connection, connection_cost, connection_length, pts = dijkstra.dijkstra_connection_roads(all_points, p1,
                            p2, c_grid_points,    line_bc,  resolution,gdf_roads, roads_segments,resolution * 30)

                ALL_POINTS = all_points.append(gdf_roads)
            elif not Roads and dist<30*resolution :
            ...
            if not connection_cost == 999999 :
                ...
                Data = {'ID1': p2['ID'].values[0], 'ID2': p1['ID'].values[0], 'C1': i, 'C2': j, 'Cost': connection_cost,
    ...
    for i in range(1,n_clusters+1):
        ...
        for index, row in Primary_substations.iterrows():
            ...
            print(p2['ID'])
            dist = p1['geometry'].values[0].distance(p2['geometry'].values[0])
            if Roads and dist<50*resolution:
                ...
                gdf_roads, roads_segments = create_roads(roads, all_points)
                connection, connection_cost, connection_length, pts = dijkstra.dijkstra_connection_roads(all_points, p1,
                            p2, c_grid_points,    line_bc,  resolution,gdf_roads, roads_segments,resolution * 50)
                ALL_POINTS = all_points.append(gdf_roads)

            elif not Roads and dist<50*resolution:
            #try:  # in case the PS is too close to the cluster
                connection, connection_cost, connection_length, pts = dijkstra.dijkstra_connection(all_points, p1, p2,
                         c_grid_points,  line_bc,   resolution,resolution*50)
...
def calculate_mg(gisele_folder,case_study,crs):
    ...
    clusters_list = [*range(n_clusters)]
    cluster_powers = [Nodes.loc[Nodes['Cluster'] == i, 'MV_Power'].sum() for i in range(n_clusters)]
    cluster_population = [Nodes.loc[Nodes['Cluster'] == i, 'MV_Power'].sum() for i in range(n_clusters)]
    ...
    yearly_profile, years, total_energy = load(clusters_list,
                                               grid_lifetime,
                                               input_profile)
    mg = sizing(yearly_profile, clusters_list, Nodes_gdf, wt, years)
    ...
    for index,row in Primary_substations.iterrows():
        ...
        if add_elevation:
            ...
            Data={'X':[row['X']],'Y': [row['Y']], 'ID': [next_ID], 'Substation':[1],'geometry': [row['geometry']],
              'Weight': [weight_of_closest], 'Cluster': [-1],'MV_Power': [0],'Elevation': [elevation_of_closest]}
        else:
            Data={'X':[row['X']],'Y': [row['Y']], 'ID': [next_ID], 'Substation':[1],'geometry': [row['geometry']],
        ...
        next_ID+=1
    return all_points
def create_input(gisele_folder,case_study,crs,line_bc,resolution,mg_option,reliability_option,Roads_option,tolerance_outside):
    ...
    if not os.path.exists(case_folder+'/Input/MILP'):
        os.makedirs(case_folder+'/Input/MILP')
        os.makedirs(case_folder + '/Input/MILP/MILP_output')
        os.makedirs(case_folder + '/Input/MILP/MILP_processed')
        os.makedirs(case_folder + '/Input/MILP/all_data')
        os.makedirs(case_folder + '/Input/MILP/MILP_input')
        os.makedirs(case_folder + '/Input/MILP/all_data/Lines_connections')
        os.makedirs(case_folder + '/Input/MILP/all_data/Lines_marked')
    if Roads_option:
        all_points = pd.read_csv(case_folder+'/Input/weighted_grid_of_points_with_ss_and_roads.csv')
    else:
        all_points = pd.read_csv(case_folder + '/Input/weighted_grid_of_points_with_ss.csv')
    ...
    # Transform to geopandas and add the primary substation as a point
    all_points = gpd.GeoDataFrame(all_points, crs=crs_str, geometry=geometry)
    next_ID = all_points.shape[0]
    ...
    All_nodes = pd.read_csv(case_folder+'/Input/MILP/all_data/All_Nodes.csv')
    ...
    All_nodes_withPS.to_csv(case_folder+'/Input/MILP/all_data/All_Nodes.csv',index=False)
    ...
    #
    create_connections(all_points_withPS,Primary_substations,gisele_folder,case_study,line_bc,resolution,crs,Roads_option,tolerance_outside)
