...
def categorize_substation(clusters_list, substations):
    ...
    clusters_list['Rated_power [kVA]'] = values
    clusters_list['Cost'] = costs
...
def locate_secondary_ss(crs, resolution, load_capita, pop_per_household, road_coef,
    ...
    for index, row in Clusters.iterrows():
        ...
        for cluster in range(max(geo_df_clustered['Cluster']) + 1):
            ...
            print(cluster)
            for index, row in geo_df_clustered_slice.iterrows():
                tot_dist = 0
                for index1, row1 in geo_df_clustered_slice.iterrows():
                    tot_dist += sqrt((row.X - row1.X) ** 2 + (row.Y - row1.Y) ** 2)
        ...
        clusters_list = pd.DataFrame(columns=['Cluster', 'Population', 'Load [kW]'])
        ...
        for i in range(int(number_clusters) + 1):
            ...
            data = np.array([[int(i), sum_pop, load]])
            df2 = pd.DataFrame(data, columns=['Cluster', 'Population', 'Load [kW]'])
        ...
        for i in range(int(number_clusters) + 1):
            load = clusters_list.loc[clusters_list['Cluster'] == i, 'Load [kW]'][0]
            population = clusters_list.loc[clusters_list['Cluster'] == i, 'Population'][0]
            load = clusters_list.loc[clusters_list['Cluster'] == i, 'Load [kW]'][0]
            grid_of_points.loc[
        ...
        clusters_list.to_csv(dir + '/clusters_list.csv')
        ...
        total_costs = sum(clusters_list['Cost'].to_list())
...
# alpha = 0.95 * alphashape.optimizealpha(new_gpd)
# hull = alphashape.alphashape(new_gpd, alpha)
# hull_buffer = hull.buffer(math.sqrt(2) * 250)
#
# # centroids
# whole_gpd['x'] = whole_gpd['geometry'].centroid.x
# whole_gpd['y'] = whole_gpd['geometry'].centroid.y
# coords = whole_gpd[["x", "y"]].to_numpy()